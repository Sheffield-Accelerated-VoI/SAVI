...
The cost-effectiveness plane shows the standardised cost-effectiveness plane per person based on `r nIterate` model runs in which uncertain model parameters are varied simultaneously in a probabilistic sensitivity analysis.  The mean incremental cost of `r input$decisionOptionCE1` versus `r input$decisionOptionCE0` is `r input$currency` `r if(!is.null(costs)) {format(incValue(costs, input$decisionOptionCE1, input$decisionOptionCE0, cache), digits=2, nsmall=2)}`.  This suggests that `r input$decisionOptionCE1` is `r if(!is.null(costs)) {moreLess(cache$costs, input$decisionOptionCE1, input$decisionOptionCE0, cache)}` costly. The incremental cost is uncertain because the model parameters are uncertain. The 97.5% credible interval for the incremental cost ranges from (`r input$currency` `r if(!is.null(costs)) {format(confIntCE(costs, input$decisionOptionCE1, input$decisionOptionCE0, 0.025, cache), digits=4,  nsmall=2)}` , `r input$currency` `r if(!is.null(costs)) {format(confIntCE(costs, input$decisionOptionCE1, input$decisionOptionCE0, 0.975, cache), digits=4,  nsmall=2)}`).  The probability that `r input$decisionOptionCE1` is cost saving compared to `r input$decisionOptionCE0` is `r if(!is.null(costs)) {format(pCostsaving(costs, input$decisionOptionCE1, input$decisionOptionCE0, cache), digits=2, nsmall=3)}`. 

The mean incremental benefit of `r input$decisionOptionCE1` versus `r input$decisionOptionCE0` is `r if(!is.null(effects)) {format(incValue(effects, input$decisionOptionCE1, input$decisionOptionCE0, cache), digits=2, nsmall=4)}` `r input$unitBens` .  Again, there is some uncertainty due to model parameters, with the 95% credible interval for the incremental benefit ranging from (`r if(!is.null(effects)) {format(confIntCE(effects, input$decisionOptionCE0, input$decisionOptionCE1, 0.025, cache), digits=4, nsmall=4)}` `r input$unitBens` , `r if(!is.null(effects)) {format(confIntCE(effects, input$decisionOptionCE0, input$decisionOptionCE1, 0.975, cache), digits=4, nsmall=4)}` `r input$unitBens`).  The probability that `r input$decisionOptionCE1` is more beneficial compared to `r input$decisionOptionCE0` is `r if(!is.null(effects)) {format(pMoreben(effects, input$decisionOptionCE1, input$decisionOptionCE0, cache), digits=2, nsmall=3)}`. 

The incremental expected cost per unit of benefit is estimated at `r input$currency` `r if(!is.null(costs)) {format(iCER(costs, effects, input$decisionOptionCE1, input$decisionOptionCE0, cache), digits=2, nsmall=2)}` per `r input$unitBens`.  There is uncertainty with a `r if(!is.null(costs)) {format(pCE(input$decisionOptionCE1, input$decisionOptionCE0, input$lambdaOverall, cache), digits=2, nsmall=3)}` probability that `r input$decisionOptionCE1` is more cost-effective than `r input$decisionOptionCE0`.  

Got this error

Quitting from lines 73-104 (report.Rmd) 
...
#     ## makes the CE plane
#     print(int <- which(cache$namesDecisions%in%intervention))
#     com <- which(cache$namesDecisions%in%comparator)
#     print(inc_costs <- costs.int[, int] - costs.int[, com])
...
#     print(m3.effects <- max(m.effects, m2.effects))
#     
...
print(groupTable)
print(dim(groupTable))
print(class(groupTable))
print(lapply(groupTable, function(x) class(x)))
#kable(as.data.frame(groupTable))
