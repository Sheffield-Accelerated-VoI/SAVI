...
if(!is.null(costs) | !is.null(effects)) {
      ...
      names<-colnames(tableCEplane[,1:min(column,4)])
       kable(tableCEplane[,1:column],col.names=names)
}
```
...
if(!is.null(costs) | !is.null(effects)) {
  tableNetBenefit<-makeTableNetBenefit(costs, effects, lambda , nInt)
      column<-length(tableNetBenefit[1,])
        kable(tableNetBenefit[,1:column])
}
```
...
if(!is.null(setStoreMatchEvpiValues)) {
 df<-matrix(setStoreMatchEvpiValues[1:3,1:6], nrow = length(setStoreMatchEvpiValues[,1]), ncol = 6)
 #print(df)
 groupEvpiTable <- matrix(NA, nrow = dim(setStoreMatchEvpiValues)[1], ncol = 6)
 #groupEvpiTable[1,1]<-df[1,1]
 #colnames(groupEvpiTable) <- c("parameters", paste("Per Person EVPPI (", input$currency, ")", sep=""), "S.Error","Indexed to Overall EVPI = 1.00", paste("EVPPI for ", input$jurisdiction, " Per Year", sep=""), paste("EVPPI for ", input$jurisdiction, " over ", input$horizon, " years", sep=""))
 #rownames(groupEvpiTable, do.NULL = TRUE)
 print(df[1[9]])
 groupEvpiTable[2,2] <- df[[1]]
 #print(groupEvpiTable)
 kable(groupEvpiTable)
